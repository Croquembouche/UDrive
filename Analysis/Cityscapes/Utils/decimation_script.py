...
def decimate_and_copy_images(source_root, destination_root, interval=10):
    """
    ...
    for city in os.listdir(source_root):
        ...
        if os.path.isdir(city_path):
            ...
            images = sorted([f for f in os.listdir(city_path) if f.lower().endswith(('png', 'jpg', 'jpeg', 'bmp', 'gif'))])

            for index, image in enumerate(images):
                if index % interval == 0:
                    src_path = os.path.join(city_path, image)
                    dest_path = os.path.join(destination_city_path, image)
...
if __name__ == "__main__":
    ...
    destination_root = "/mnt/nas/Cityscapes/yolo/decimated_images/train"  # Change this to your desired destination root
